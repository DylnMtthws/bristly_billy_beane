...
> **ANSWERED FROM THE CODE, 2026-09-08.** The first task below is done; the
> finding is recorded here so nobody re-investigates it. See
> `RESEARCH_ASSISTANT_SPEC.md` §14.1.
>
> **The absence is silent, and production is in the silent case.** `lab.py`
> returns `None` from `_summarise_evidence` (line 314) and `_explain` (line 342)
> when `gateway is None`, **without appending a warning**; only `_intent`
> (line 267) warns, and only when `raw_intent` is set — an explicit `pack_id`
> returns at line 256 before the gateway check, which is the UI's normal path
> with one supported pack. `templates/cedh/candidate.html:164` is
> `{% if explanation %}` with **no `{% else %}`**, and `cedh_routes.py:candidate`
> does not pass `modes` to the template. So the candidate page renders no
> explanation, no evidence summary and no warning. `LabModes.notices` covers this
> only on `lab.html`, a different page seen before the build.
>
> **This is an `absence_is_visible` violation and it is fixable here, with no
> dependency on the provider decision.** It is scheduled into the Research
> Assistant's R0. Note the contrast that makes it a bug rather than a design:
> when a credential *is* present and the provider *fails*, `lab.py:_call` appends
> a warning and `candidate.html:19` renders it. The degradation path is correct;
> the never-configured path is not.
>
> **Requirement 5 is also substantially satisfied**: `config/settings.yaml` sets
> `monthly_cost_ceiling_usd: 15.0`, and `_call` catches `LLMCostCeilingExceeded`
> and surfaces it as a warning rather than logging and ignoring it.
>
> **PROVIDER DECIDED, 2026-09-08.** The owner has a HuggingFace account with
> billing enabled. **Item 2 is settled with no code changes:** keep the
> HuggingFace router, keep `deepseek-ai/DeepSeek-V4-Flash:deepinfra`, keep
> `credential_env: HF_TOKEN`. The `:deepinfra` suffix is router syntax that
> `provider_deepseek.py:115` `_assert_pinned` requires, and the G3 bake-off wants
> several models under one credential — both arguments for the router over a
> direct DeepInfra account.
>
> **What remains is items 3–4, and they are owner actions rather than decisions:**
>
> 1. `fly secrets set HF_TOKEN=… --app dylnmtthws-decklab` (this restarts the app).
> 2. `python scripts/smoke_model_gateway.py` — walks all five acceptance criteria
>    in order and prints measured tokens, cost and latency. Steps 1, 2, 4 and 5
>    are verified offline; step 3 needs the real token.
> 3. Record the measured cost against the estimate in the spec's §11, which is
>    unvalidated arithmetic until one real call lands a `cost_log` row.
...
> **CENSUS SPECIFIED AND MADE EXECUTABLE, 2026-09-08.**
> [`d1-corpus-census.md`](d1-corpus-census.md) records the scope (six measures,
> and what is deliberately excluded), the counting method with the reason each
> choice changes the answer, the figures already on record, and seven numbered
> uncertainties. `scripts/d1_census.py` runs the eight queries in one
> `REPEATABLE READ READ ONLY` snapshot, checks every one through
> `assert_v1_only`, warns if the role is not `mtg_consumer`, and **refuses to run
> without a DSN** rather than falling back to fixtures.
>
> **Only the run is outstanding**, and it needs a live `mtg_consumer` DSN that a
> feature worktree correctly does not hold. Commit the JSON beside the doc and
> fill in its "Measured" table.
...
> **SCOPED FROM THE CODE, 2026-09-08.** D3 is **not a blocker**; R6 ships
> without it. Recorded here so the cost/benefit is not re-derived.
>
> **v3 already supplies more than "counts plus warning strings."**
> `coverage.inert_by_reason` breaks the inert set across a closed five-category
> set — `interaction`, `opponent_trigger`, `opponent_permanent`, `timing_only`,
> `no_object_in_model` — alongside `modeled_cards`, `inert_cards` and
> `unauthored_cards`, and `strategy_pack.known_blind_spots` carries the pack's
> declared limits as prose. Deck Lab validates v3 today
> (`cedh/simulator.py:RESULT_SCHEMA_ID`) and reads all of it. So the capability
> statement can already say *"12 invisible because the model has no opponents, 9
> with no object in the model, 10 unauthored"* — it just cannot name a card.
>
> **The case for v4 is that the data already exists inside the simulator.** It
> prints an inert table in the human run report, which is the only reason
> `Hullbreaker Horror`'s misclassification is documented at all. v4 is a
> serialization change, not new analysis — and a known misclassification that is
> in a human report but not in the machine contract is visible to a person and
