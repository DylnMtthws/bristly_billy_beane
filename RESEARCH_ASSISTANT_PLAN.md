...
These modules are pure functions over card text with no DB, no config and no
casual-power-level judgment. They move to `mechanics/`:
...
| `role_tagger.py` | Role classification from text. Audit for scoring weights first; if it carries any, they stay behind. |
