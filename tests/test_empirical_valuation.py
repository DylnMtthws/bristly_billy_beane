...
def test_empirical_boost_raises_score_and_absence_is_neutral() -> None:
    ...
    boosted = compute_synergy_score(
        card,
        _base_ctx(
            empirical_inclusion={"pitiless plunderer": 0.85},
    ...
    absent = compute_synergy_score(
        card,
        _base_ctx(empirical_inclusion={"some other card": 0.9}),
    )
    assert absent == base
...
def test_reliable_inclusion_boosts_more_than_noisy() -> None:
    ...
    reliable = compute_synergy_score(
        card,
        _base_ctx(
            empirical_inclusion={"deadly dispute": 0.6},
...
def test_get_target_cluster_inclusion_selects_and_degrades(
    ...
    inc2 = ev.get_target_cluster_inclusion(
        tmp_path / "x.db", "Cmdr", strategy="landfall"
    )
...
def test_pooled_fallback_merges_same_archetype_clusters(tmp_path, monkeypatch) -> None:
    ...
    decks = [
        DeckRecord(deck_id=f"a{i}", card_names=ARISTO + ["Filler A"], popularity_rank=i)
        for i in range(36)
    ] + [
        DeckRecord(deck_id=f"b{i}", card_names=ARISTO + ["Filler B"], popularity_rank=i)
        for i in range(36)
    ]
    monkeypatch.setattr(ev, "load_commander_decks", lambda *a, **k: decks)
