...
def _valuation() -> CommanderValuation:
    ...
    return CommanderValuation(
        ...
        clusters=[
            ClusterValuation(
                cluster_id=0,
                dominant_archetype="aristocrats",
                size=49,
                meets_floor=True,
                staples=[staple],
...
def test_build_clusters_block_includes_stats_and_sample() -> None:
    members = {
        0: [
            DeckRecord(
                deck_id="d1",
                card_names=["Sol Ring", "Viscera Seer", "Korvold"],
                popularity_rank=1,
            ),
...
def test_response_model_parses_expected_shape() -> None:
    data = {
        "variants": [
            {
                "cluster_id": 0,
                "variant_name": "Sacrifice-value",
                "game_plan": "Grind value via sac loops.",
                "key_cards": ["Viscera Seer"],
                "differentiators": "More sac outlets.",
