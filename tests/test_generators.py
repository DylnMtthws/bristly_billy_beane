...
def _make_ramp_pool() -> list[dict]:
    ...
    for i in range(10):
        cards.append(
            {
                "id": f"signet-{i}",
                "name": f"Test Signet {i}",
                "type_line": "Artifact",
                "oracle_text": "{T}: Add {W} or {U}.",
                "price_usd": 0.5,
                "cmc": 2,
                "_cvar_score": 0.5 + i * 0.02,
                "role_tags": '["ramp"]',
            }
        )
    return cards
...
def _make_removal_pool() -> list[dict]:
    ...
    for i in range(12):
        ...
        cards.append(
            {
                "id": f"removal-{i}",
                "name": f"Test Removal {i}",
                "type_line": "Instant" if not is_wipe else "Sorcery",
                "oracle_text": (
                    "Destroy all creatures." if is_wipe else f"Destroy target {target}."
                ),
                "price_usd": 1.0,
                "cmc": 3,
                "_cvar_score": 0.5 + i * 0.02,
...
def test_draw_generator_prefers_repeatable() -> None:
    ...
    repeatable = [
        a for a in result if "enchantment" in (a.card.get("type_line") or "").lower()
    ]
...
def test_removal_generator_includes_board_wipes() -> None:
    ...
    def _is_wipe(card: dict) -> bool:
        return card.get("removal_type") == "board_wipe" or "board_wipe" in (
            card.get("role_tags") or ""
        )
...
def test_ramp_generator_never_places_lands() -> None:
    ...
    assert all("land" not in (a.card.get("type_line") or "").lower() for a in result)
...
def test_land_generator_subtracts_already_placed_lands() -> None:
    ...
    gen = LandPackageGenerator(_NOT_A_DB)
    placed_lands = [
        {"name": f"Stray Land {i}", "type_line": "Land", "cmc": 0} for i in range(2)
    ]
    result = gen.generate(
        color_identity=["W", "U"],
        target_count=36,
...
def test_suspend_rock_scores_far_below_real_rock() -> None:
    ...
    assert (
        _score_ramp(sol_ring, ["W", "B"], 3.0)
        > _score_ramp(sol_talisman, ["W", "B"], 3.0) + 0.15
    )
